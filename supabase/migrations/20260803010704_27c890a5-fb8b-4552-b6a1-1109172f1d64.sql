...
REVOKE EXECUTE ON FUNCTION public.apply_material_movement() FROM PUBLIC;
...
DROP POLICY IF EXISTS "waitlist all authenticated" ON public.waitlist;
CREATE POLICY "waitlist all authenticated" ON public.waitlist FOR ALL TO authenticated 
  USING (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'staff'))
  WITH CHECK (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'staff'));
